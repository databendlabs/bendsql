...
#[no_mangle]
pub extern "system" fn Java_com_databend_bendsql_AsyncExecutor_makeTokioExecutor(
    mut env: JNIEnv,
    _: JClass,
    cores: usize,
) -> jlong {
...
#[no_mangle]
pub unsafe extern "system" fn Java_com_databend_bendsql_AsyncExecutor_disposeInternal(
    _: JNIEnv,
    _: JObject,
...
pub(crate) fn make_tokio_executor(env: &mut JNIEnv, cores: usize) -> Result<Executor> {
    ...
    let executor = tokio::runtime::Builder::new_multi_thread()
        ...
        .thread_name_fn(move || {
            ...
            format!("databend-jdbc-tokio-worker-{}", id)
        })
        ...
        .build()
        .map_err(|e| {
            databend_driver::Error::Unexpected(format!("Failed to create tokio runtime: {e}"))
            //.set_source(e)
        })?;
